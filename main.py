...
def add_parser_arguments(parser):
    ...
    parser.add_argument('--arch', '-a', metavar='ARCH', default='efficientnet-b7',
                        choices=model_names,
...
def main(args):
    ...
    from EfficientNet import efficientnet_params
    if 'efficient' in args.arch:
        _,_,image_size,_ = efficientnet_params(args.arch)
    else :
        image_size = 224

    train_loader, train_loader_len = get_train_loader(args.data, image_size,args.batch_size, 1000, args.mixup > 0.0, workers=args.workers, fp16=args.fp16)
    if args.mixup != 0.0:
        train_loader = MixUpWrapper(args.mixup, 1000, train_loader)

    val_loader, val_loader_len = get_val_loader(args.data, image_size,args.batch_size, 1000, False, workers=args.workers, fp16=args.fp16)
